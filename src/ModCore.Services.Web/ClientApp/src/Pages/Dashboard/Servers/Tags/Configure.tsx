...
export default function Configure({
    ...
    const [selectedTagHistory, setSelectedTagHistory] = useState<DatabaseTag | null>(null);
    const [deleteConfirmationOpen, setDeleteConfirmationOpen] = useState(false);
    const [tagToDelete, setTagToDelete] = useState<DatabaseTag | null>(null);
    const [takeOwnershipConfirmationOpen, setTakeOwnershipConfirmationOpen] = useState(false);
    const [tagToTakeOwnership, setTagToTakeOwnership] = useState<DatabaseTag | null>(null);
    ...
    const handleTakeOwnership = () => {
        if (tagToTakeOwnership) {
            ...
            const updatedTag = { ...tagToTakeOwnership, author_id: Number(user?.id) ?? 0 };
            setTags(tags.map((tag) => (tag.id === updatedTag.id ? updatedTag : tag)));
            setFilteredTags(filteredTags.map((tag) => (tag.id === updatedTag.id ? updatedTag : tag)));
            ...
            <DashboardLayout>
                ...
                <Card className="w-full bg-gray-900 p-6 rounded-lg shadow-lg">
                    ...
                    <CardContent>
                        <Table>
                            <TableCaption>
                                A list of all tags for this server.
                            </TableCaption>
                            <TableHeader>
                                <TableRow>
                            ...
                            <TableBody>
                                {paginatedTags.map((tag) => (
                                    <TableRow key={tag.id}>
                                        ...
                                        <TableCell>#{channels.find(x => x.id == tag.channel_id)?.name ?? ""}</TableCell>
                                        <TableCell>{tag.content}</TableCell>
                                        <TableCell>
                                            <Dialog>
                                                <DialogTrigger asChild>
                                                    <Button variant="outline" size="sm" onClick={() => setSelectedTagHistory(tag)}>
                                                        View History
                                                    </Button>
                                                </DialogTrigger>
                                                <DialogContent className="sm:max-w-[425px]">
                                                    ...
                                                    {selectedTagHistory && selectedTagHistory.history && selectedTagHistory.history.length > 0 ? (
                                                        <Table>
                                                            <TableHeader>
                                                                <TableRow>
                                                            ...
                                                            <TableBody>
                                                                {selectedTagHistory.history.map((historyItem, index) => (
                                                                    <TableRow key={index}>
                                                                        <TableCell>{new Date(historyItem.timestamp).toLocaleString()}</TableCell>
                                            ...
                                            <div className="flex gap-2">
                                                <AlertDialog>
                                                    <AlertDialogTrigger asChild>
                                                        <Button variant="destructive" size="sm" onClick={() => setTagToDelete(tag)}>
                                                            Delete
                                                        </Button>
                                                    </AlertDialogTrigger>
                                                    <AlertDialogContent>
                                                        <AlertDialogHeader>
                                                            ...
                                                            <AlertDialogDescription>
                                                                This action cannot be undone. Are you sure you want to delete tag "{tag.name}"?
                                                            </AlertDialogDescription>
                                                        </AlertDialogHeader>
                                                        <AlertDialogFooter>
                                                ...
                                                <AlertDialog>
                                                    <AlertDialogTrigger asChild>
                                                        <Button variant="outline" size="sm" onClick={() => setTagToTakeOwnership(tag)}>
                                                            Take Ownership
                                                        </Button>
                                                    </AlertDialogTrigger>
                                                    <AlertDialogContent>
                                                        <AlertDialogHeader>
                                                            <AlertDialogTitle>Take Ownership?</AlertDialogTitle>
                                                            <AlertDialogDescription>
                                                                Are you sure you want to take ownership of tag "{tag.name}"?
                                                            </AlertDialogDescription>
                                                        </AlertDialogHeader>
                                                        <AlertDialogFooter>
                                                            <AlertDialogCancel onClick={() => setTagToTakeOwnership(null)}>Cancel</AlertDialogCancel>
                                                            <AlertDialogAction onClick={handleTakeOwnership}>Continue</AlertDialogAction>
                                                        </AlertDialogFooter>
                                                    </AlertDialogContent>
                                                </AlertDialog>
                                            </div>
