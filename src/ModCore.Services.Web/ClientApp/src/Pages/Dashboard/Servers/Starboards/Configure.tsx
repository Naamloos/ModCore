...
export default function Configure({
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        // post(`/dashboard/servers/${server.id}/starboards`, data);
    };
    ...
    const handleInputChange = (
        ...
        newStarboards[index][e.target.name] = e.target.value;
        setData("starboards", newStarboards);
    };

    const handleSwitchChange = (index: number, e: React.ChangeEvent<HTMLInputElement>) => {
        const newStarboards = [...(data.starboards as any)];
        newStarboards[index]["enabled"] = e.target.checked;
        setData("starboards", newStarboards);
    };
                ...
                <Card className="w-full bg-gray-900 p-6 rounded-lg shadow-lg">
                    ...
                    <CardContent>
                        <form onSubmit={handleSubmit}>
                            {(data.starboards as any)?.map((starboard: Starboard, index: number) => (
                                <div
                                    key={index}
                                ...
                                >
                                    <div className="w-full flex items-center justify-between mb-2">
                                        <span className="text-lg font-semibold text-white">
                                            Starboard (ID: {starboard.id})
                                        </span>
                                        <Button
                                            variant="destructive"
                                            size="icon"
                                            onClick={() => {removeStarboard(index)}}
                                    ...
                                    <div className="grid gap-4 mb-4">
                                        <div>
                                            ...
                                            <Input
                                                ...
                                                value={starboard.minimum_reactions}
                                                onChange={(e) => handleInputChange(index, e)}
                                            />
                                            <p className="mt-2 text-sm text-gray-500">
                                                The minimum number of reactions required
                                                for a message to be added to the
                                                starboard.
                                            </p>
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            />
                                            <p className="mt-2 text-sm text-gray-500">
                                                The emoji used to trigger the starboard.
                                            </p>
                                        </div>
                                        ...
                                        <div>
                                            <DiscordChannelPicker
                                                ...
                                                onUpdate={(val) => {
                                                    const newStarboards = [...(data.starboards as any)];
                                                    newStarboards[index]["channelId"] = val;
                                                    setData("starboards", newStarboards);
                                                }}
                                                required
                                            />
                                            <p className="mt-2 text-sm text-gray-500">
                                                The channel where starboard messages
                                                will be sent.
                                            </p>
                                        </div>
                                    </div>
                            ...
                            <Button type="button" variant="secondary" onClick={addStarboard} className="ml-2 mb-4">
                                Add Starboard
                            </Button>
                        </form>
                    </CardContent>
