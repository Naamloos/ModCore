...
export default function DiscordMessageVisualizerProxy({embeds, content, className}: {embeds?: DiscordEmbed[], content: string, className: string})
{
    const [parsedEmbeds, setParsedEmbeds] = useState<DiscordEmbed[] | undefined>(embeds);
    const [parsedContent, setParsedContent] = useState<string>(content);
    const { application, user } = usePage<{ application: DiscordApplication, user: User}>().props;
    ...
    useEffect(() => {
        if (embeds)
        {
            let newEmbeds = embeds.map(embed => overridePlaceholdersInEmbed(embed));
