...
{
    [Route("dashboard/servers/{server_id}/banappeal")]
    [RouteMiddleware(typeof(RequireAuthentication))]
    [ApiController]
    public class BanAppealsController : ControllerBase
    {
        [RouteMiddleware(typeof(RequireAuthentication))]
        [HttpGet]
        public async Task<IActionResult> BanAppeals([FromRoute] ulong server_id, [FromServices] DatabaseContext database)
        {
            ...
            var appeals = database.BanAppeals.Where(x => x.GuildId == server_id).ToList();

            return Inertia.Render("Dashboard/Servers/BanAppeals/Configure", new
            {
                ...
                Appeals = JsonSerializer.SerializeToDocument(appeals, options: serializerOptions),
            });
        }
    }
}
