...
{
    [Route("dashboard/servers/{server_id}/infractions")]
    ...
    public class InfractionController : ControllerBase
    {
        ...
        public async Task<IActionResult> ServerTagsSettings([FromRoute] ulong server_id, [FromServices] DatabaseContext database)
        {
            ...
            var infractions = database.Infractions.Where(x => x.GuildId == server_id).ToList();

            return Inertia.Render("Dashboard/Servers/Infractions/Configure", new
            {
                ...
                Infractions = JsonSerializer.SerializeToDocument(infractions, options: serializerOptions),
            });
        }
