...
{
    [Route("dashboard/servers/{server_id}/autorole")]
    [RouteMiddleware(typeof(RequireAuthentication))]
    [ApiController]
    public class AutoRoleConfigController : ControllerBase
    {
        [RouteMiddleware(typeof(RequireAuthentication))]
        [HttpGet]
        public async Task<IActionResult> AutoRoleSettings([FromRoute] ulong server_id, [FromServices] DatabaseContext database)
        {
            ...
            return Inertia.Render("Dashboard/Servers/AutoRole/Configure", new
            {
