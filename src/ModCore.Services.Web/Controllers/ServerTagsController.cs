...
{
    [Route("dashboard/servers/{server_id}/tags")]
    ...
    public class ServerTagsController : ControllerBase
    {
        ...
        public async Task<IActionResult> ServerTagsSettings([FromRoute] ulong server_id, [FromServices] DatabaseContext database)
        {
            ...
            var tags = database.Tags.Where(x => x.GuildId == server_id).ToList();

            return Inertia.Render("Dashboard/Servers/Tags/Configure", new
            {
                ...
                Tags = JsonSerializer.SerializeToDocument(tags, options: serializerOptions),
            });
        }
