        ...
        public static async Task<TimerData> RescheduleTimers(DiscordClient client, DatabaseContextBuilder database, SharedData shared)
        {
            ...
            try
            {
                ...
                if (CancelIfLaterThan(nearest.DispatchAt, shared, force))
                {
                    ...
                    var dispatch = nearest.DispatchAt - DateTimeOffset.UtcNow;

                    if (dispatch.TotalMilliseconds > Int32.MaxValue)
                    {
                        using (var db = database.CreateContext())
                        {
                        ...
                        return await RescheduleTimers(client, database, shared);
                    }

                    var task = Task.Delay(dispatch, cts.Token);
