...
namespace IronBlock.Blocks.Controls
{
	public class ControlsIf : IBlock
	{
        public override async Task<object> EvaluateAsync(Context context)
		{
			...
			{
				if ((bool)await Values.EvaluateAsync($"IF{i}", context))
				{
		...
		public override SyntaxNode Generate(Context context)
		{
			...
			for (var i = 0; i < ifCount; i++)
			{
				...
				var ifContext = new Context(context.Dependency) { Parent = context };
			...
			if (elseMutation == "1")
			{
				...
				var elseContext = new Context(context.Dependency) { Parent = context };
				if (statement?.Block != null)
				{
